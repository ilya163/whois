...
class saveExcel:
    ...
    def run(self, response):
        try:
            ...
            # logger.info("Данные в формате Excel успешно записаны")
        except Exception as err:
            logger.error(f"Ошибка при запуске обработчка Excel: {str(err)}")
    ...
    def xls_worker(self, ws, response):
        try:
            ...
            logger.error(f"Ошибка в воркере Excel {str(err)}")
